...
A generic Java binary search tree with node lookup, depth, height, and balance checks.

## Technical depth

Recursive algorithms, tree invariants, and the cost of repeated subtree traversal.
...
Use BinarySearchTree from Java. This copy adds a linear-time balance check and fixes detection of deep imbalances.
...
The repackaged source compiled with JDK 24. The local JUnit run passed **7 tests**, with 0 failures. Maven configuration is provided for convenience; the reported run used javac and JUnit Console directly. Passing tests do not establish exhaustive correctness or production readiness.
